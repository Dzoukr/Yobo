...
let toEvent (cryptoProvider:SymetricCryptoProvider) = function
    ...
    | "PasswordReset", data -> data |> Serialization.objectFromJToken<CmdArgs.ResetPassword> |> PasswordReset
...
let toData (cryptoProvider:SymetricCryptoProvider) = function
    ...
    | PasswordReset args -> "PasswordReset", (args |> Serialization.objectToJToken)
