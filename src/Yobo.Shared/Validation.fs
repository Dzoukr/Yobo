...
type ValidationError = 
    ...
    with
        member x.Explain() =
            ...
            | TermsNotAgreed _ -> "Potvrďte souhlas s obchodními podmínkami"
...
let validateTermsAgreed txt getter args =
    let value = args |> getter
    if value = false then TermsNotAgreed(txt) |> Some else None
